                        ...
                        <a data-toggle="modal" href="#myModal">
                            <div class="btn-group">
                                <button id="" class="btn green">
                                  <i class="fa fa-plus-circle"></i>  Add Bed Category 
                                </button>
                            </div>
                        </a>
                        ...
                        <?php foreach ($categories as $category) { ?>
                            <tr class="">
                                ...
                                <td>
                                    ...
                                    <a class="btn btn-info btn-xs btn_width delete_button" href="bed/deleteBedcategory?id=<?php echo $category->id; ?>" onclick="return confirm('Are you sure you want to delete this item?');"><i class="fa fa-trash-o"></i></a>
                                </td>
                            </tr>
                        <?php } ?>
                        </tbody>
...
<div class="modal fade" id="myModal2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form role="form" id="editCategoryForm" action="bed/addCategory" method="post" enctype="multipart/form-data">
...
<script type="text/javascript">
                                    $(document).ready(function () {
                                        $(".editbutton").click(function (e) {
                                            ...
                                            $('#myModal2').modal('show');
                                            $.ajax({
                                                url: 'bed/editCategoryByJason?id=' + iid,
                                                method: 'GET',
                                                data: '',
                                                dataType: 'json',
                                            }).success(function (response) {
                                                // Populate the form fields with the data returned from server
                                                $('#editCategoryForm').find('[name="id"]').val(response.bedcategory.id).end()
                                                //   $('#editEventForm').find('[name="p_id"]').val(response.client.client_id).end()
                                                $('#editCategoryForm').find('[name="category"]').val(response.bedcategory.category).end()
                                                $('#editCategoryForm').find('[name="description"]').val(response.bedcategory.description).end()
                                            });
