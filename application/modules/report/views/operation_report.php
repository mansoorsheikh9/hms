...
<div class="modal fade" id="myModal2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form role="form" id="editOperationReportForm" action="report/addReport" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        <label for="exampleInputEmail1">Select Type</label>
                        <select class="form-control m-bot15" name="type" value=''>
                            <option value="birth">Birth</option>
...
<script type="text/javascript">
                                    $(document).ready(function () {
                                        $(".editbutton").click(function (e) {
                                            ...
                                            $('#editOperationReportForm').trigger("reset");
                                            $('#myModal2').modal('show');
                                            $.ajax({
                                                ...
                                                $('#editOperationReportForm').find('[name="id"]').val(response.report.id).end()
                                                //   $('#editEventForm').find('[name="p_id"]').val(response.client.client_id).end()
                                                $('#editOperationReportForm').find('[name="description"]').val(response.report.description).end()
                                                $('#editOperationReportForm').find('[name="patient"]').val(response.report.patient).end()
                                                $('#editOperationReportForm').find('[name="doctor"]').val(response.report.doctor).end()
                                                $('#editOperationReportForm').find('[name="date"]').val(response.report.date).end()
                                            });
